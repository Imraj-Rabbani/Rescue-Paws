...
const ProductPage = () => {
    ...
    const handleSaveProduct = async () => {
        try {
            ...
            setIsEditing(false);
            setNewProduct({ id: '', name: '', description: '', purchaseCost: 0, sellingPrice: 0, imageUrl: '', stockQuantity: 0 });
    ...
    const handleIncreaseStock = async (productId) => {
        const updatedProducts = products.map(product => {
            ...
            return product; });  setProducts(updatedProducts);  };
                            ...
                            <table className="min-w-full">
                                <thead className={isDarkMode ? "bg-gray-600" : "bg-[#e4ccb8]"}>
                                    <tr>
                                        ...
                                        <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-300' : 'text-[#664C36]'}`}>Description</th>
                                        <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-300' : 'text-[#664C36]'}`}>Purchase Cost</th>
                                        <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-300' : 'text-[#664C36]'}`}>Selling Price</th>
                                ...
                                <tbody className={`divide-y ${isDarkMode ? 'divide-gray-700 bg-gray-900' : 'divide-gray-200 bg-white'}`}>
                                    {products.map((product) => (
                                        <tr key={product.id}>
                                            ...
                                            <td className={`px-6 py-4 whitespace-normal text-sm ${isDarkMode ? 'text-white' : 'text-[#664C36]'}`}>{product.description}</td>
                                            <td className={`px-6 py-4 whitespace-nowrap text-sm ${isDarkMode ? 'text-white' : 'text-[#664C36]'}`}>BDT {product.purchaseCost?.toFixed(2)}</td>
                                            <td className={`px-6 py-4 whitespace-nowrap text-sm ${isDarkMode ? 'text-white' : 'text-[#664C36]'}`}>BDT {product.sellingPrice?.toFixed(2)}</td>
                                        ...
                                        <div className="grid gap-4 py-4">
                                            <div className="grid grid-cols-4 items-center gap-4">
                                                ...
                                                <input
                                                    ...
                                                    required
                                                    readOnly={isEditing} // Make ID read-only when editing 
                                                /> </div>
                                            <div className="grid grid-cols-4 items-center gap-4">
